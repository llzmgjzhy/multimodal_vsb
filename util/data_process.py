...
def getArgparse():
    ...
    parser.add_argument(
        ...
        type=int,
        default=160,
        help="Number of chunks to divide the data into",
    )
    parser.add_argument(
        "--output_path",
        type=str,
...
if __name__ == "__main__":
    ...
    # read csv file
    meta_df = pd.read_csv(args.data_dir + "metadata_train.csv")
    signal_ids = meta_df["signal_id"].values

    # flatten the data and detect peaks
    signal_df = pq.read_pandas(args.data_dir + "train.parquet").to_pandas()
    _, all_flat_signals, all_points = peaks_on_flatten(signal_df, signal_ids)

    ##### Denoise & Extract Waveforms #####
    # construct waveforms with given window size
    waveforms = choose_chunk_peak(
        all_flat_signals,
        all_points,
    )
    ...
    pickle.dump(
        waveforms,
        open(
            args.output_path + "all_chunk_waves_{}chunks.dat".format(args.nchunks), "wb"
        ),
    )
